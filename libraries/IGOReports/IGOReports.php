...
class IGOReports {
	...
	public static function init($config = APPPATH . 'modules/reports/config/reports.php') {

		//set up our autoloader
		spl_autoload_register(array('IGOReports','loader'),true,true);
		...
		if(!file_exists(APPPATH . 'modules/reports/config/reports.php.sample')) {
			throw new Exception("Cannot find sample config. Please leave config/config.php.sample in place for default values.");
		}

		$default_config = include(APPPATH . 'modules/reports/config/reports.php.sample');
		...
		self::$requestbase = $protocol.rtrim($_SERVER['HTTP_HOST'].base_url(),'/'); //.($_SERVER['SERVER_PORT']!=$port?':'.$_SERVER['SERVER_PORT']:"");
		...
		self::$twig->addFunction(new Twig_SimpleFunction('dbdate', 'IGOReports::dbdate'));
        self::$twig->addFunction(new Twig_SimpleFunction('sqlin', 'IGOReports::generateSqlIN'));
        ...
        self::$twig_string->addFunction(new Twig_SimpleFunction('sqlin', 'IGOReports::generateSqlIN'));

		$CI = &get_instance();
		$CI->load->driver('cache');
		$CI->cache->file->save($cacheKey, $data, 60 * 60 * 4); // 4 hr
	...
	public static function render($template, $macros) {
		...
		$default = array(
			'base'=>self::$requestbase,
			'report_list_url'=>self::$requestbase.'/',
	...
	public static function displayReport($report,$type) {		
		...
		try {
			...
			$r=is_object($report)? $report->report: 'na';
			echo self::render('html/page',array(
				'title'=>$r,
				'header'=>'<h2>'.$error_header.'</h2>',
				'error'=>$e->getMessage(),
				'content'=>$content,
				'breadcrumb'=>array('Report List'=>'', $r => true)
			));
		}
	}
	...
	public static function getDashboards() {
		$dashboards = glob(IGOReports::$config['dashboardDir'].'/*.json');
	...
	public static function getDashboard($dashboard) {
		$file = IGOReports::$config['dashboardDir'].'/'.$dashboard.'.json';
		if(!file_exists($file)) {
	...
	public static function getRecentReports() {
		$CI = &get_instance();
		$CI->load->driver('cache');
		$recently_run = $CI->cache->get('recently_run');
		$recent = array();
	...
	public static function getReportListJSON($reports=null) {
		...
		$CI = &get_instance();
		$CI->load->driver('cache');
		$CI->cache->get($cacheKey, $data, 60 * 60 * 4); // 4 hr

		$recently_run = SystemCache::retrieve(SystemCache::generateCacheKey('recently_run'));
		$popular = array();
		if($recently_run !== false) {
			foreach($recently_run as $report) {
		...
		foreach($reports as $report) {
			if($report['is_dir'] && isset($report['children']) && $report['children']) {
				...
				$r = isset($report['report'])?$report['report']:'';
				if(isset($popular[$r])) {
					$popularity = $popular[$report['report']];
				}
				else $popularity = 0;

				$u = isset($report['url'])?$report['url']:'';
	...
	protected static function getReportHeaders($report) {
		...
		if(!isset($_REQUEST['nocache'])) {
			$CI = &get_instance();
			$CI->load->driver('cache');
			memCache::retrieve('recently_run');
		...
		if($data === false) {
			...
			$data['report'] = $report;
			$data['url'] = self::$requestbase.'/report/html/?report='.$report;
			$data['is_dir'] = false;
	...
	protected static function getReports($dir, &$errors = null) {
		...
		foreach($reports as $key=>$report) {
			...
			if(is_dir($report)) {
				...
				$bn=basename($report);
				$return[] = array(
					'Name'=>ucwords(str_replace(array('_','-'),' ',$bn)),
					'Title'=>$title,
					'Id'=> $id,
	...
	public static function emailReport() {		
		...
		if(!isset(IGOReports::$config['mail_settings']['enabled']) || !IGOReports::$config['mail_settings']['enabled']) {
			echo json_encode(array('error'=>'Email is disabled on this server'));
			return;
		}
		if(!isset(IGOReports::$config['mail_settings']['from'])) {
			echo json_encode(array('error'=>'Email settings have not been properly configured on this server'));
			return;
		}
		
		$from = IGOReports::$config['mail_settings']['from'];
	...
	protected static function getMailTransport() {
		if(!isset(IGOReports::$config['mail_settings'])) IGOReports::$config['mail_settings'] = array();
		if(!isset(IGOReports::$config['mail_settings']['method'])) IGOReports::$config['mail_settings']['method'] = 'mail';
		
		switch(IGOReports::$config['mail_settings']['method']) {
			case 'mail':
				return Swift_MailTransport::newInstance();
			case 'sendmail':
				return Swift_MailTransport::newInstance(
					isset(IGOReports::$config['mail_settings']['command'])? IGOReports::$config['mail_settings']['command'] : '/usr/sbin/sendmail -bs'
				);
			case 'smtp':
				if(!isset(IGOReports::$config['mail_settings']['server'])) throw new Exception("SMTP server must be configured");
				$transport = Swift_SmtpTransport::newInstance(
					IGOReports::$config['mail_settings']['server'],
					isset(IGOReports::$config['mail_settings']['port'])? IGOReports::$config['mail_settings']['port'] : 25
				);
				
				//if username/password
				if(isset(IGOReports::$config['mail_settings']['username'])) {
					$transport->setUsername(IGOReports::$config['mail_settings']['username']);
					$transport->setPassword(IGOReports::$config['mail_settings']['password']);
				}
				
				//if using encryption
				if(isset(IGOReports::$config['mail_settings']['encryption'])) {
					$transport->setEncryption(IGOReports::$config['mail_settings']['encryption']);
				}
